...
def encode_word(word: str) -> List[float]:
    ...
    # Test is the first letter is uppercase or not
    if word[0].isupper():
        features.append(1)
    else:
        features.append(0)

    # , period ? bip bop, bop bip?
    if '.' in word:
        features.append(1)
    else:
        features.append(0)

    # Prefixes TIME HAS CAME
...
class MEMM:
    ...
    def __init__(self, tag_list: List[str]):
        ...
        self.perceptron = [[random.random() for i in range(nb_feat+1)] for j in range(len(tag_list))]
    ...
    def encode(self, pword: str, ptag: str, cword: str, fword: str) -> List[float]:
        ...
        I'll be completly honest it's too much :'((
        """

        features = []

        # is the past tag equals to "< t >"? (yes:1, no:0)
        if ptag == "<t>":
            features.append(1)
        else:
            features.append(0)

        # onehot encoding of the tag
        features += onehot(self.tag_list, ptag)

        # is the past word equals to "< s >"? (yes:1, no:0)
        if pword == "<s>":
            features.append(1)
        else:
            features.append(0)

        # vector encoding of past word using the past function
        if pword == "<s>":
            features += [0] * 50  # Use a zero vector for the start token
        else:
            features += encode_word(pword)

        # vector encoding of current word using the past function
        features += encode_word(cword)

        # is the future word equals to "< /s >"? (yes:1, no:0)
        if fword == "</s>":
            features.append(1)
        else:
            features.append(0)

        # vector encoding of future word using the past function
        if fword == "</s>":
            features += [0] * 50  # Use a zero vector for the end token
        else:
            features += encode_word(fword)
...
class CKY:
    ...
    def parse(self, sentence: List[str]) -> List[List[List[Tuple[str, int, int, int]]]]:
        n = len(sentence)
        table = [[[] for _ in range(n)] for _ in range(n)]

        for i, tag in enumerate(self.lex.predict_all(sentence)):
            table[i][i].append((tag, -1, 0, 0))
